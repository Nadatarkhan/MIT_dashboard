document.addEventListener('DOMContentLoaded', function() {
    ...
    // Creating a narrow left panel specifically for the grid icon
    const leftPanel = document.createElement('div');
    leftPanel.className = 'left-panel';
    leftPanel.style.width = '100px'; // Set the width for the left panel
    leftPanel.style.height = container.clientHeight + 'px'; // Match the height of the container
    leftPanel.style.position = 'absolute';
    leftPanel.style.left = '0';
    leftPanel.style.top = '0';
    leftPanel.style.backgroundColor = '#f0f0f0'; // Example background color
    container.appendChild(leftPanel);

    const dpi = window.devicePixelRatio;
    const containerWidth = container.clientWidth - 150; // Reduce width to account for left panel
    ...
    const canvas = d3.select(container)
        ...
        .style("left", "100px"); // Adjust left position to accommodate left panel
    ...
    d3.csv("data/example_data.csv").then(function(data) {
        ...
        fields.forEach(field => {
            const targetContainer = field === 'grid' ? leftPanel : container; // Use leftPanel for 'grid', else use main container
